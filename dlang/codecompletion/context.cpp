...
QList<CompletionTreeItemPointer> DCodeCompletionContext::completionItems(bool& abort, bool fullCompletion)
{
    ...
    if (m_text.lastIndexOf(QChar::fromLatin1(';')) != -1)
        m_text = m_text.mid(m_text.lastIndexOf(QChar::fromLatin1(';')));

    // TODO: Fix for D.
    if (m_text.contains(QRegularExpression(QStringLiteral("import.*$")))) {
...
bool DCodeCompletionContext::isInsideCommentOrString()
{
    ...
    QString text = QChar::fromLatin1(' ') + m_fullText;
